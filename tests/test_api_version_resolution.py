...
The detection mechanism lives in veeam_365.discovery and is tested there. What matters here
...
api_version.py imports Home Assistant, which this environment does not have, so
...
def entry(**overrides):
    data = {"host": "vb365.example.com", "port": 4443, "api_version": AUTO}
...
def test_auto_resolves_to_the_detected_version():
    """Detection answers in module names; the entry stores display versions."""
    resolver = load_resolver(detected="v7")

    assert resolve(resolver, entry()) == "7"
...
def test_detection_is_told_only_supported_versions():
    ...
    record = {}
    resolver = load_resolver(detected="v8", record=record)

    resolve(resolver, entry())

    assert record["versions"] == list(SUPPORTED.values())
    assert record["base_url"] == "https://vb365.example.com:4443"
...
def test_undetectable_server_falls_back_to_the_default():
    """The server may be behind a proxy that rewrites statuses; setup should still proceed."""
    resolver = load_resolver(detected=None)

    assert resolve(resolver, entry()) == DEFAULT


def test_an_unknown_detected_module_falls_back_to_the_default():
    """A version the server serves but this integration cannot map is no answer at all."""
    resolver = load_resolver(detected="v99")
...
def test_missing_api_version_is_treated_as_auto():
    ...
    resolver = load_resolver(detected="v8")

    assert resolve(resolver, data) == "8"
...
def test_the_sentinel_is_stored_not_resolved_away():
    ...
    Resolving it at save time would freeze the entry on whichever version was newest that
    day, which defeats the point: a server upgrade or a newer veeam-365 should move it on.
    """
    ...
    options = flow[flow.index("class Veeam365OptionsFlow") :]
    assert (
...
def _load_const():
    ...
    spec = importlib.util.spec_from_file_location("veeam_365_const", CONST_PATH)
...
def test_the_resolved_version_wins_once_setup_has_run():
    const = _load_const()
    entry = Entry(data={"api_version": "auto"}, runtime_data={"api_version": "6"})

    assert const.configured_api_version(entry) == "6"
...
def test_a_pinned_version_is_returned_as_is():
    ...
    assert const.configured_api_version(Entry(data={"api_version": "7"})) == "7"


def test_options_override_data():
    const = _load_const()
    entry = Entry(data={"api_version": "6"}, options={"api_version": "7"})

    assert const.configured_api_version(entry) == "7"
...
def test_module_names_map_back_to_display_versions():
    """Detection answers in module names, which is not what a config entry stores."""
    const = _load_const()

    for display, module in const.API_VERSIONS.items():
        assert const.display_version_for_module(module) == display

    assert const.display_version_for_module("v99") is None


def test_versions_are_offered_newest_first():
    """The version most people want should be at the top of the dropdown."""
    const = _load_const()

    offered = [int(display) for display in const.API_VERSIONS]
